        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-start">
          <div>
            ...
            <h2
              ...
              className="text-[clamp(36px,4.5vw,58px)] font-bold uppercase tracking-tight text-black mb-6"
            >
              READY TO COVER YOUR MATCH?
            </h2>
            <p className="text-black/50 text-[15px] leading-relaxed mb-10">
            ...
            <div className="flex flex-col gap-5">
              {[
                ...
                <div key={item.label} className="flex items-start gap-4">
                  ...
                  <div>
                    <div style={{ fontFamily: "'Clash Display', sans-serif" }} className="text-[10px] tracking-widest uppercase text-black/35 mb-0.5">{item.label}</div>
                    <div className="text-[14px] text-black/65">{item.val}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Form card */}
          <div className="bg-[#f8f8f8] border border-black/8 p-10">
            {submitted ? (
              <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                <div className="text-5xl">🏏</div>
                <h3 style={{ fontFamily: "'Clash Display', sans-serif" }} className="text-2xl font-bold uppercase text-black">Message Sent!</h3>
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="flex flex-col gap-5">
                <div className="grid grid-cols-2 gap-4">
                  {[{ label: "Name *", type: "text", ph: "Your name" }, { label: "Phone *", type: "tel", ph: "+91..." }].map((f) => (
                    <div key={f.label}>
                      ...
                      <input required type={f.type} placeholder={f.ph} className="w-full bg-white border border-black/15 text-black text-[13px] px-4 py-3.5 outline-none placeholder:text-black/25 focus:border-[#004aad] transition-colors" />
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <input required type="email" placeholder="your@email.com" className="w-full bg-white border border-black/15 text-black text-[13px] px-4 py-3.5 outline-none placeholder:text-black/25 focus:border-[#004aad] transition-colors" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select className="w-full bg-white border border-black/15 text-black/70 text-[13px] px-4 py-3.5 outline-none focus:border-[#004aad] transition-colors" style={{ appearance: "none" }}>
                ...
                <div>
                  ...
                  <textarea rows={3} placeholder="Tournament name, dates, location, expected audience..." className="w-full bg-white border border-black/15 text-black text-[13px] px-4 py-3.5 outline-none placeholder:text-black/25 resize-none focus:border-[#004aad] transition-colors" />
                </div>
                <button
                  type="submit"
